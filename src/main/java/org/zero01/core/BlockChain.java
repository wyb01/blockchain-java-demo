...
public class BlockChain {
	...
	public Map<String, Object> newBlock(long proof, String previous_hash) {
		...
		block.put("index", getChain().size() + 1);   //索引
		block.put("timestamp", System.currentTimeMillis());  //时间戳
		block.put("transactions", getCurrentTransactions());  //交易列表
		block.put("proof", proof);  //工作量证明
		// 如果没有传递上一个区块的hash就计算出区块链中最后一个区块的hash
		block.put("previous_hash", previous_hash != null ? previous_hash : hash(getChain().get(getChain().size() - 1)));  //前一区块hash值

		// 重置当前的交易信息列表
		setCurrentTransactions(new ArrayList<Map<String, Object>>());

		getChain().add(block); //区块链

		return block;  //返回新建的区块链
	}
	...
	public int newTransactions(String sender, String recipient, long amount) {
		...
		transaction.put("sender", sender);        //发送方的地址
		transaction.put("recipient", recipient);  //接收方得地址
		transaction.put("amount", amount);        //交易数量

		getCurrentTransactions().add(transaction);  //添加到"交易信息列表"
	 ...
	 * 规则是：寻找一个数 p，使得它与前一个区块的 proof 拼接成的字符串的 Hash 值以 4 个零开头
