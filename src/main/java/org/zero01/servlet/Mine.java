...
//该Servlet用于运行工作算法的证明来获得下一个证明，也就是所谓的挖矿(挖出一个新的区块来存储这笔交易)
@WebServlet("/mine")
public class Mine extends HttpServlet {

	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		BlockChain blockChain = BlockChain.getInstance();  //创建单例对象
		Map<String, Object> lastBlock = blockChain.lastBlock();  //得到区块链中的最后一个区块
		long lastProof = Long.parseLong(lastBlock.get("proof") + "");  //工作量
		long proof = blockChain.proofOfWork(lastProof);   //简单的工作量证明
